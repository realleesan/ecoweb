...
/**
 * Convert datetime-local input (Y-m-d\TH:i) to MySQL DATETIME string.
 */
...
/**
 * Parse numeric input supporting localized separators.
 */
...
try {
    ...
    try {
        ...
        $couponTableAvailable = false;
        $errors[] = 'Bảng dữ liệu "coupons" chưa tồn tại. Vui lòng chạy migration tương ứng trước khi thêm mã giảm giá.';
    }
} catch (Throwable $exception) {
    $errors[] = 'Không thể kết nối tới cơ sở dữ liệu. Vui lòng thử lại sau.';
}

if ($pdo && $couponTableAvailable && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $formValues = [
    ...
    if (!$errors) {
        $codeCheck = $pdo->prepare('SELECT COUNT(*) FROM coupons WHERE coupon_code = :coupon_code');
        $codeCheck->execute([':coupon_code' => $couponCode]);
    ...
    if (!$errors) {
        try {
            $insert = $pdo->prepare('INSERT INTO coupons (coupon_code, coupon_name, description, discount_type, discount_value, max_discount_value, min_order_value, usage_limit, per_customer_limit, status, start_date, end_date) VALUES (:coupon_code, :coupon_name, :description, :discount_type, :discount_value, :max_discount_value, :min_order_value, :usage_limit, :per_customer_limit, :status, :start_date, :end_date)');
            $insert->execute([
            ...
            $successMessage = 'Tạo mã giảm giá mới thành công.';
            $formValues = [
                'coupon_code' => '',
                'coupon_name' => '',
                'description' => '',
            ...
            $errors[] = 'Không thể tạo mã giảm giá. Vui lòng thử lại.';
        }
    }
}
...
<div class="admin-layout">
    ...
    <main class="coupon-create" aria-labelledby="coupon-create-title">
        <div class="coupon-create__header">
            <h1 class="coupon-create__title" id="coupon-create-title">Thêm mã giảm giá</h1>
        ...
        <?php if ($errors): ?>
            <div class="notice notice--error" role="alert">
                ...
                <div>
                    <strong>Không thể tạo mã giảm giá:</strong>
                    <ul>
        ...
        <form method="post" class="coupon-form" novalidate>
            <div class="coupon-card">
                <h2 class="coupon-card__title">Thông tin cơ bản</h2>
            ...
            <div class="coupon-actions">
                <button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Tạo mã giảm giá</button>
